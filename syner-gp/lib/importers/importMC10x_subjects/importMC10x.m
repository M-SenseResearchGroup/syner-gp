...
if nargin > 0
    ...
    potential_fields = {'locationNames','sensorLocations','locations','location','locationName','sensorLocation'};
    has_field = isfield(importOptions,potential_fields);
    ifield = find(has_field);
    ...
    potential_fields = {'storeSameTrials','storeSameActivities','sameActivites','sameTrials','importSameTrials','importSameActiities'};
    has_field = isfield(importOptions,potential_fields);
    ifield = find(has_field);
    ...
    potential_fields = {'renameTrials','rename'};
    has_field = isfield(importOptions,potential_fields);
    ifield = find(has_field);
...
if isa(option.renameTrials,'char')
    ...
    if ~strcmpi(option.renameTrials,'none')
        ...
        % then reset flag and change to a cell element
        renameFlag = 1;
        option.renameTrials = {option.renameTrials};
            ...
            for l = 1:length(option.locationNames)
                ...
                for s = 1:length(option.sensorNames)
                    ...
                    if isfile(file_name)
                        ...
                        data.trials.(trialNames{t})(f).locations.(option.locationNames{l}).(option.sensorNames{s}).data = d(iunique,2:end)';
